...
let compute_ca32 x result =
  Dsl.(extract 31 0 result < extract 31 0 x)
...
let add_dot mode rt ra rb =
  ...
  Dsl.[
    rt := var ra + var rb;
    nf := is_negative mode rt;
    pf := is_positive mode rt;
    zf := is_zero mode rt;
    so := var so lor var ov;
  ]
...
let addic mode rt ra imm =
  let rt = find_gpr rt in
  let ra = find_gpr ra in
  let imm =
    Word.of_int64 ~width:gpr_bitwidth (Imm.to_int64 imm) in
  let tmp = Var.create ~fresh:true "tmp" (Type.imm gpr_bitwidth) in
  Dsl.[
    tmp := var ra;
    rt := var ra + int imm;
    ca := compute_ca mode (var tmp) (var rt);
...
let addze_dot mode rt ra =
  ...
  Dsl.[
    tmp := var ra;
    rt := var ra + cast unsigned gpr_bitwidth (var ca);
    ca := compute_ca mode (var ra) (var rt);
    ca32 := compute_ca32 (var ra) (var rt);
    nf := is_negative mode rt;
    pf := is_positive mode rt;
    zf := is_zero mode rt;
    so := var so lor var ov;
...
let lift opcode mode endian mem ops = match opcode, ops with
  ...
  | `ADDIC,  [| Reg rt; Reg ra; Imm imm |] -> addic mode rt ra imm
  | `ADDICo, [| Reg rt; Reg ra; Imm imm |] -> addic_dot mode rt ra imm
  | `ADDC,   [| Reg rt; Reg ra; Reg rb  |] -> addc mode rt ra rb
  | `ADDCo,  [| Reg rt; Reg ra; Reg rb  |] -> addc_dot mode rt ra rb
  | `ADDE,   [| Reg rt; Reg ra; Reg rb  |] -> adde mode rt ra rb
  | `ADDEo,  [| Reg rt; Reg ra; Reg rb  |] -> adde_dot mode rt ra rb
  | `ADDME,  [| Reg rt; Reg ra;         |] -> addme mode rt ra
  | `ADDMEo, [| Reg rt; Reg ra;         |] -> addme_dot mode rt ra
  | `ADDZE,  [| Reg rt; Reg ra;         |] -> addze mode rt ra
